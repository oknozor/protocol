...
pub fn write_variant(plan: &plan::Enum)
    ...
    let variant_match_branches: Vec<_> = plan.variants.iter().map(|variant| {
        ...
        let write_discriminator_stmt = quote! { <#discriminator_ty as djin_protocol::Parcel>::write(#discriminator_ref_expr, __io_writer, __settings)?; };
        ...
        quote!(#enum_name :: #fields_pattern => {
            ...
            #(
                djin_protocol::Parcel::write_field(#binding_names, __io_writer, __settings, &mut __hints)?;
            )*
        })
    }).collect();
...
pub fn read_variant(plan: &plan::Enum)
    ...
    quote! {
        {
            let discriminator: #discriminator_ty = djin_protocol::Parcel::read_field(__io_reader, __settings, &mut __hints)?;
